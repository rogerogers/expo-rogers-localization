...
public class LocalizationModule: Module {
  public func definition() -> ModuleDefinition {
    ...
    OnCreate {
      if let forceRTL = Bundle.main.object(forInfoDictionaryKey: "ExpoLocalization_forcesRTL")
        as? Bool
      {
        self.setRTLPreferences(true, forceRTL)
      } else {
        if let enableRTL = Bundle.main.object(forInfoDictionaryKey: "ExpoLocalization_supportsRTL")
          as? Bool
        {
  ...
  func isRTLPreferredForCurrentLocale() -> Bool {
    if UserDefaults.standard.object(forKey: "RCTI18nUtil_forceRTL") != nil {
      return UserDefaults.standard.bool(forKey: "RCTI18nUtil_forceRTL")
    }
    // swiftlint:disable:next legacy_objc_type
    return NSLocale.characterDirection(forLanguage: NSLocale.preferredLanguages.first ?? "en-US")
  ...
  func setRTLPreferences(_ supportsRTL: Bool, _ forceRTL: Bool) {
    ...
    if forceRTL {
      ...
      UserDefaults.standard.set(
        supportsRTL ? isRTLPreferredForCurrentLocale() : false, forKey: "RCTI18nUtil_forceRTL")
    }
  ...
  static func getUnicodeCalendarIdentifier(calendar: Calendar) -> String {
    ...
    @unknown default:
      log.error(
        "Unhandled `Calendar.Identifier` value: \(calendar.identifier), returning `iso8601` as fallback. Add the missing case as soon as possible."
      )
      return "iso8601"
    }
  }
      ...
      .map { languageTag -> [String: Any?] in
        ...
        if #available(iOS 16, tvOS 16, *) {
          return [
            ...
            "textDirection": languageLocale.language.characterDirection == .rightToLeft
              ? "rtl" : "ltr",
        ...
        return [
          ...
          "textDirection": Locale.characterDirection(forLanguage: languageTag) == .rightToLeft
            ? "rtl" : "ltr",
          "decimalSeparator": userSettingsLocale.decimalSeparator,
          "digitGroupingSeparator": userSettingsLocale.groupingSeparator,
          "measurementSystem": getMeasurementSystemForLocale(userSettingsLocale),
